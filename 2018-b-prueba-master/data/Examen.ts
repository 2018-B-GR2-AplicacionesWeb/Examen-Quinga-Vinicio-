...
inicializarBDD()
    .then(respuesta => { //bdd no reconoce
        respuesta.bdd.map(valor => {
...
inicializarBDD()
    .then(respuesta => { //bdd no reconoce
        respuesta.bdd.map(valor => {
            switch (valor.skin_color) {
                case 'fair':
                    arreglof.push(valor.skin_color);
                    break;
                case 'gold':
                    arreglog.push(valor.skin_color);
                    break;
                case 'white':
                    arreglow.push(valor.skin_color);
                    break;
                case 'light':
                    arreglol.push(valor.skin_color);
                    break
            }
        });
